...
def moh_facility(request):
    ...
    return render(
        request,
        "ssme_activities/moh_facility.html",
        {
            "cds_form": cds_form,
...
def get_pop_total(request, code=""):
    ...
    if not code:
        return pop_total.values("population_cible").aggregate(
            population_cible=Sum("population_cible")
        )
    if not pop_total:
        pop_total = {}
        ...
        return (
            pop_total.filter(cds__district__province__code=int(code))
            .values("population_cible")
            .aggregate(population_cible=Sum("population_cible"))
        )
    if len(code) > 2 and len(code) <= 4:
        return (
            pop_total.filter(cds__district__code=int(code))
            .values("population_cible")
            .aggregate(population_cible=Sum("population_cible"))
        )
    if len(code) > 4:
        return (
            pop_total.filter(cds__code=int(code))
            .values("population_cible")
            .aggregate(population_cible=Sum("population_cible"))
        )
        ...
        if "cds" in kwargs:
            ...
            if not queryset_benef:
                ...
                for t in headers_benef:
                    ress = (
                        queryset_benef.annotate(
                            beneficiaires=Concat(
                                Substr(
                                    F(
                                ...
                                V(" ("),
                                Substr(
                                    F("campaign_beneficiary__beneficiary__designation"),
                                    1,
                                    5,
                                ),
                                V(")"),
                        ...
                        .filter(
                            reception_date__lte=today["reception_date"],
                            beneficiaires=t["beneficiaires"],
                        )
                        .values("received_number")
                    )
                    if not ress:
            ...
            queryset_benef = queryset_benef.filter(
                report__cds__district=kwargs.get("district").id
            )
        elif "province" in kwargs:
            queryset_benef = queryset_benef.filter(
                report__cds__district__province=kwargs.get("province").id
            )
        if not queryset_benef:
            return []
        else:
            for t in headers_benef:
                ...
                if not ress:
                    ...
                    ress = reduce(
                        lambda x, y: dict((k, v + y[k]) for k, v in x.iteritems()), ress
                    )
                    res.update({t["beneficiaires"]: ress["received_number"]})
                body_benef.update(res)
            return body_benef
        ...
        for i in dates_benef:
            ...
            for t in headers_benef:
                ress = (
                    ...
                    .filter(
                        reception_date=i["reception_date"],
                        beneficiaires=t["beneficiaires"],
                    )
                    .values("received_number")
                    .aggregate(total=Sum("received_number"))
                )
                if not ress["total"]:
                    res.update({t["beneficiaires"]: 0})
                else:
...
def get_reception(queryset_reception, dates_reception, headers_recept, **kwargs):
    ...
    if not dates_reception:
        res, ress = today, {}
        if "cds" in kwargs:
            queryset_reception = queryset_reception.filter(
                report__cds=kwargs.get("cds").id
            )
        elif "district" in kwargs:
            queryset_reception = queryset_reception.filter(
                report__cds__district=kwargs.get("district").id
        ...
        if not queryset_reception:
            ...
            for t in headers_recept:
                ress = (
                    queryset_reception.annotate(
                        products=F("campaign_product__product__name")
                    )
                    .filter(
                        reception_date__lte=today["reception_date"],
                        products=t["products"],
        ...
        for i in dates_reception:
            ...
            for t in headers_recept:
                ress = (
                    ...
                    .filter(reception_date=i["reception_date"], products=t["products"])
                    .values("received_quantity")
                    .aggregate(total=Sum("received_quantity"))
                )
                if not ress["total"]:
...
def get_remain_cds(queryset_remain, dates_remain, headers_recept, **kwargs):
    if not dates_remain:
        ...
        for t in headers_recept:
            ress = queryset_remain.annotate(
                products=F("campaign_product__product__name")
            ).filter(products=t["products"])
            if not ress:
        ...
        for t in headers_recept:
            ress = (
                queryset_remain.annotate(products=F("campaign_product__product__name"))
                .filter(products=t["products"])
                .values("remain_quantity")
                .aggregate(total=Sum("remain_quantity"))
            )
            if not ress["total"]:
                rest.update({str(t["products"]): 0})
            else:
...
def get_remain(queryset_remain, dates_remain, headers_recept, **kwargs):
    if not dates_remain:
        if "cds" in kwargs:
            ...
            cds_r = get_remain_cds(
                queryset_remain, dates_remain, headers_recept, cds=kwargs.get("cds")
            )
            return cds_r
        elif "district" in kwargs:
            district_r = []
            queryset_remain = queryset_remain.filter(
                report__cds__district=kwargs.get("district")
            )
            ...
            for i in queryset_remain.values("report__cds__district").distinct():
                ...
                somme = get_remain(
                    queryset_remain, dates_remain, headers_recept, district=district
                )
                for i in somme:
                    del i["cds"]
        ...
        for d in dates_remain:
            ...
            for i in queryset.values("report__cds").distinct():
                ...
                if queryset_temp:
                    bb.append(
                        get_remain_cds(
                            queryset_temp, [], headers_recept, cds=i["report__cds"]
                        )
                    )
            for i in bb:
                del i["cds"]
...
class ProvinceDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not pop_total:
            ...
            pop_total = pop_total.values("population_cible").aggregate(
                population_cible=Sum("population_cible")
            )
        context["pop_total"] = pop_total
        # benef
        headers_benef = (
            CampaignBeneficiaryProduct.objects.all()
            .annotate(
                beneficiaires=Concat(
                    Substr(F("campaign_product__product__name"), 1, 10),
                    V(" ("),
                    Substr(F("campaign_beneficiary__beneficiary__designation"), 1, 5),
                    V(")"),
                    output_field=CharField(),
                )
            )
        ...
        for district in districts:
            res = get_benef(
                queryset_benef, dates_today, headers_benef, district=district
            )
            if res == []:
                pass
            else:
        ...
        headers_recept = (
            ...
            .annotate(products=F("product__name"))
            .values("products")
            .distinct()
            .order_by("order_in_sms")
        )
        queryset_reception = get_report_by_code(
            self.request, mycode, ReportProductReception
        )
        body_reception = []
        for district in districts:
            res = get_reception(
                queryset_reception, dates_today, headers_recept, district=district
            )
            if res == []:
                pass
            else:
        ...
        queryset_remain = get_report_by_code(
            self.request, mycode, ReportProductRemainStock
        )
        body_remain = get_remain(
            queryset_remain, dates_today, headers_recept, province=context["object"]
        )
...
class DistrictDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for cds in cdss:
            res = get_reception(
                queryset_reception, dates_today, headers_recept, cds=cds
            )
            if res == []:
                pass
            else:
        ...
        body_remain = get_remain(
            queryset_remain, dates_today, headers_recept, district=context["object"]
        )
        context["body_remain"] = body_remain
        context["taux"] = get_per_category_taux(self.request)
        context["recus"] = total_received(self.request, mycode)
...
class CDSDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        queryset_benef = get_report_by_code(self.request, mycode, ReportBeneficiary)
        dates_benef = (
            queryset_benef.values("reception_date")
            .distinct()
        ...
        dates_reception = (
            queryset_reception.values("reception_date")
            .distinct()
            .order_by("reception_date")
        )
        body_reception = []
        if queryset_reception:
            body_reception = get_reception(
                queryset_reception, dates_reception, headers_recept
            )
        # Remain
        queryset_remain = get_report_by_code(
            self.request, mycode, ReportProductRemainStock
        ...
        dates_remain = (
            queryset_remain.values("concerned_date")
            .distinct()
            .order_by("concerned_date")
        )
        body_remain = []
        if queryset_remain:
            body_remain = get_remain(
                queryset_remain, dates_remain, headers_recept, cds=context["object"]
            )
        context["body_benef"] = body_benef
        context["body_reception"] = body_reception
        context["body_remain"] = body_remain
...
class UserSignupView(CreateView):
    ...
    def form_valid(self, form):
        ...
        profile.save()
        if (
            form["user"].cleaned_data["password1"] == ""
            or form["user"].cleaned_data["password2"] == ""
        ):
            try:
                ...
                messages.success(
                    self.request,
                    _("Profile created and mail sent to %(email)s.")
                    % {"email": user.email},
                )
            except:
                messages.success(
                    self.request,
                    _("Unable to send mail  to %(email)s.") % {"email": user.email},
                )
                pass
...
class CampaignCRUDL(SmartCRUDL):
    ...
    class Create(SmartCreateView):
        ...
        def form_valid(self, form):
            messages.success(
                self.request,
                _("Campaign created %(email)s.") % {"email": form.cleaned_data["name"]},
            )
...
class CampaignBeneficiaryProductCRUDL(SmartCRUDL):
    ...
    class List(SmartListView):
        search_fields = (
            "campaign_beneficiary__beneficiary__designation__icontains",
            "campaign_product__product__name__icontains",
        )
...
class CampaignCDSCRUDL(SmartCRUDL):
    ...
    class List(SmartListView):
        fields = (
            ...
            "population_cible",
            "cds.district.name",
            "cds.district.province.name",
        )
        search_fields = (
            "cds__name__icontains",
            "cds__district__name__icontains",
            "cds__district__province__name__icontains",
        )
        default_order = "cds"
...
def initialise_data(request, **kwargs):
    ...
    if (
        not mycode["mycode"]
        and not request.user.groups.filter(name="CEN").exists()
        and not request.user.is_superuser
    ):
        messages.warning(
            request,
            _(
                "You have no valid MoH facility attached to \
    ...
    return {
        "mycode": mycode,
        "pop_total": pop_total,
        "headers_benef": headers_benef,
        "headers_recept": headers_recept,
        "taux": taux,
    }
...
def get_reports(request, **kwargs):
    ...
    return render(
        request,
        "ssme_activities/reports.html",
        {
            "body_benef": body_benef,
            "headers_benef": headers_benef,
            "headers_recept": headers_recept,
            "body_reception": body_reception,
...
def get_reports_by_benef(request, **kwargs):
    ...
    return render(
        request,
        "ssme_activities/reports_by_benef.html",
        {
            "body_benef": body_benef["benefs"],
            "headers_benef": headers_benef,
            "pop_total": pop_total,
            "taux": taux,
        },
...
def get_reports_by_received(request, **kwargs):
    ...
    return render(
        request,
        "ssme_activities/reports_by_received.html",
        {
            "body_reception": body_reception,
            "headers_benef": headers_benef,
            "headers_recept": headers_recept,
...
def get_reports_by_remaining(request, **kwargs):
    ...
    return render(
        request,
        "ssme_activities/reports_by_remaining.html",
        {
            "body_remain": body_remain,
            "headers_benef": headers_benef,
            "recus": recus,
...
def get_reports_by_rates(request, **kwargs):
    ...
    return render(
        request,
        "ssme_activities/reports_by_rates.html",
...
def get_benef_in_json(request):
    ...
    data = json.dumps(
        [
            dict(item)
            for item in benef.annotate(
                beneficiaires=F("campaign_beneficiary__beneficiary__designation")
            )
            .annotate(province=F("report__cds__district__province__name"))
            .annotate(pop_servie=F("received_number"))
            .annotate(cds=F("report__cds__name"))
...
def get_recus_in_json(request):
    ...
    data = json.dumps(
        [
            dict(item)
            for item in recus.annotate(products=F("campaign_product__product__name"))
            .annotate(province=F("report__cds__district__province__name"))
            .annotate(quantite_recue=F("received_quantity"))
            .annotate(district=F("report__cds__district__name"))
            .annotate(cds=F("report__cds__name"))
            .values(
                "products",
...
def get_final_in_json(request):
    ...
    data = json.dumps(
        [
            dict(item)
            for item in finals.annotate(products=F("campaign_product__product__name"))
            .annotate(province=F("report__cds__district__province__name"))
            .annotate(quantite_restante=F("remain_quantity"))
            .annotate(district=F("report__cds__district__name"))
            .annotate(cds=F("report__cds__name"))
            .values(
                "products",
...
def estimate(request, cds=""):
    headers_benef = (
        CampaignBeneficiary.objects.filter(campaign__going_on=True)
    ...
    for h in headers_benef:
        benef = ReportBeneficiary.objects.filter(
            report__cds__code=cds,
            campaign_beneficiary__beneficiary__designation=h["beneficiaires"],
        ).aggregate(Sum("received_number"))
        for i in CampaignBeneficiaryProduct.objects.filter(
            campaign_beneficiary__beneficiary__designation=h["beneficiaires"]
        ):
            if benef["received_number__sum"]:
                benef.update(
                    {
                        str(i.campaign_product.product.name): int(
                            i.dosage * benef["received_number__sum"]
                        )
                    }
                )
            else:
...
def total_received(request, mycode=""):
    headers_recept = (
        CampaignProduct.objects.filter(campaign__going_on=True)
        .annotate(products=F("product__name"))
        .values("products")
        .distinct()
    )
    ...
    for h in headers_recept:
        recu = queryset_recu.filter(
            campaign_product__product__name=h["products"]
        ).aggregate(Sum("received_quantity"))
...
def fetchbeneficiaries(request):
    ...
    if camp_id:
        ...
        data = Beneficiaire.objects.filter(
            campaignbeneficiary__campaign=the_last_campaign
        )
...
def participation(request):
    ...
    target_population_for_this_campaign = CampaignCDS.objects.filter(
        campaign=the_last_campaign
    ).aggregate(Sum("population_cible"))
    ...
    if the_last_campaign:
        beneficiaries_4_last_campaign = Beneficiaire.objects.filter(
            campaignbeneficiary__campaign=the_last_campaign
        )

        if beneficiaryid.isdigit():
            related_campaign_beneficiaries = (
                CampaignBeneficiary.objects.filter(
                    campaign=the_last_campaign, beneficiary__id=beneficiaryid
                )
                .annotate(
                    received_people=Sum(
                        "campaignbeneficiaryproduct__reportbeneficiary__received_number"
                    )
                )
                .values()
            )
            ...
            related_campaign_beneficiaries = (
                CampaignBeneficiary.objects.filter(campaign=the_last_campaign)
                .annotate(
                    received_people=Sum(
                        "campaignbeneficiaryproduct__reportbeneficiary__received_number"
                    )
        ...
        response_data = json.dumps(
            list(related_campaign_beneficiaries), cls=DjangoJSONEncoder
        )
        ...
        for r in rows:
            ...
            r["target_population"] = target_population_for_this_campaign[
                "population_cible__sum"
            ]

            received_number_on_first_date = ReportBeneficiary.objects.filter(
                beneficiaries_per_product__campaign_beneficiary__id=r["id"],
                reception_date=the_camp_start_date,
            ).aggregate(Sum("received_number"))
            if received_number_on_first_date:
                r["received_on_day_one"] = received_number_on_first_date[
                    "received_number__sum"
                ]

                received_number_on_day_2 = ReportBeneficiary.objects.filter(
                    beneficiaries_per_product__campaign_beneficiary__id=r["id"],
                    reception_date=date_of_day_two,
                ).aggregate(Sum("received_number"))
                if received_number_on_day_2:
                    r["received_on_day_two"] = (
                        received_number_on_day_2["received_number__sum"]
                        + r["received_on_day_one"]
                    )

                    received_number_on_day_3 = ReportBeneficiary.objects.filter(
                        beneficiaries_per_product__campaign_beneficiary__id=r["id"],
                        reception_date=date_of_day_three,
                    ).aggregate(Sum("received_number"))
                    if received_number_on_day_3:
                        r["received_on_day_three"] = (
                            received_number_on_day_3["received_number__sum"]
                            + r["received_on_day_two"]
                        )

                        received_number_on_day_4 = ReportBeneficiary.objects.filter(
                            beneficiaries_per_product__campaign_beneficiary__id=r["id"],
                            reception_date=date_of_day_four,
                        ).aggregate(Sum("received_number"))
                        if received_number_on_day_4:
                            r["received_on_day_four"] = (
                                received_number_on_day_4["received_number__sum"]
                                + r["received_on_day_three"]
                            )
                        else:
            ...
            if beneficiaryid.isdigit() and beneficiaryid > 0:
                beneficiary_target = CampaignBeneficiary.objects.get(
                    campaign=the_last_campaign, beneficiary__id=beneficiaryid
                ).pourcentage_attendu

                if r["received_on_day_one"]:
                    r["percentage_on_day_one"] = round(
                        (
                            r["received_on_day_one"]
                            / (beneficiary_target * r["target_population"] / 100)
                        )
                        * 100,
                        2,
                    )
                ...
                if r["received_on_day_three"]:
                    r["percentage_on_day_three"] = round(
                        (
                            r["received_on_day_three"]
                            / (beneficiary_target * r["target_population"] / 100)
                        )
                ...
                if r["received_on_day_four"]:
                    r["percentage_on_day_four"] = round(
                        (
                            r["received_on_day_four"]
                            / (beneficiary_target * r["target_population"] / 100)
                        )
