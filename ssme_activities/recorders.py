...
def check_number_of_values(args):
    ...
    if args["message_type"] == "SELF_REGISTRATION":
        if len(args["text"].split()) < 3:
            ...
            args[
                "info_to_contact"
            ] = "Erreur. Vous avez envoye peu de valeurs. Veuillez reenvoyer le message corrige."
        if len(args["text"].split()) > 3:
            args["valide"] = False
            args[
                "info_to_contact"
            ] = "Erreur. Vous avez envoye beaucoup de valeurs. Veuillez reenvoyer le message corrige."
        if len(args["text"].split()) == 3:
            args["valide"] = True
...
def identify_the_opened_campaign(args):
    ...
    if len(campaign) > 1:
        ...
        args[
            "info_to_contact"
        ] = "Erreur admin. Plusieurs campagnes ouvertes en meme temps."
    if len(campaign) == 1:
...
def check_date_is_in_camp_period(args):
    ...
    sent_date = (
        args["text"].split()[1][0:2]
        + "-"
        + args["text"].split()[1][2:4]
        + "-20"
        + args["text"].split()[1][4:]
    )
    ...
    if (
        date_sent < args["opened_campaign"].start_date
        or date_sent > args["opened_campaign"].end_date
    ):
        args["valide"] = False
        args[
            "info_to_contact"
        ] = "Erreur. La date indiquee n est pas dans la periode de la campagne SSME ouverte."
        return
...
def identify_number_of_concerned_products(args):
    ...
    if len(campaign_products) < 1:
        ...
        args[
            "info_to_contact"
        ] = "Erreur admin. Pas de produits lies a la campagne ouverte."
        return
...
def check_product_values_validity(args):
    ...
    while (priority <= args["number_of_concerned_products"]) and (priority > 0):
        ...
        campaign_product = CampaignProduct.objects.filter(
            campaign=args["opened_campaign"], order_in_sms=priority
        )

        if len(campaign_product) < 1:
            args["valide"] = False
            args["info_to_contact"] = (
                "Erreur admin. Pas de produits de priorite " + str(priority) + "."
            )
            priority = -1
        else:
...
def check_cds(args):
    ...
    if len(concerned_cds) > 0:
        ...
        args[
            "info_to_contact"
        ] = "Erreur. Le code cds envoye n est pas enregistre dans le systeme."
...
def check_supervisor_phone_number(args):
    ...
    if re.search(expression, the_supervisor_phone_number_no_space) is None:
        ...
        args[
            "info_to_contact"
        ] = "Erreur. Le numero de telephone du superviseur n est pas bien ecrit."
    else:
        args["valide"] = True
        args[
            "info_to_contact"
...
def save_temporary_the_reporter(args):
    ...
    if len(same_existing_temp) > 0:
        ...
        args[
            "info_to_contact"
        ] = "Erreur. Vous devriez envoyer le numero de telephone de votre superviseur seulement."
    else:
        ...
        if len(cds) > 0:
            ...
            args[
                "info_to_contact"
            ] = "Merci. Veuillez confirmer le numero de telephone du superviseur s il vous plait."
    ...
    if len(the_existing_temp) < 1:
        ...
        if (
            ...
            if len(check_duplication) > 0:
                ...
                args[
                    "info_to_contact"
                ] = "Erreur. Vous etes deja enregistre sur ce CDS et avec le meme numero de telephone du superviseur. Envoyer votre rapport ou X pour sortir."
            ...
            check_duplication = Reporter.objects.filter(
                ~Q(cds=the_one_existing_temp.cds),
                phone_number=the_one_existing_temp.phone_number,
                supervisor_phone_number=the_one_existing_temp.supervisor_phone_number,
            )
            if len(check_duplication) > 0:
                ...
                args["valide"] = True
                args["info_to_contact"] = (
                    "Mise a jour du CDS reussie. Votre nouveau CDS est : "
                    + the_one_existing_temp.cds.name
                )
                the_one_existing_temp.delete()
                return
            ...
            if len(check_duplication) > 0:
                ...
                args["valide"] = True
                args["info_to_contact"] = (
                    "Mise a jour reussie. Le nouveau numero de telephone de votre superviseur est : "
                    + the_one_existing_temp.supervisor_phone_number
                    + ". Merci."
                )
                the_one_existing_temp.delete()
            ...
            if len(check_duplication) > 0:
                ...
                args["valide"] = True
                args["info_to_contact"] = (
                    "Mise a jour reussie. Le nouveau numero de votre superviseur est : "
                    + the_one_existing_temp.supervisor_phone_number
                    + " et le nouveau CDS est :"
                    + the_one_existing_temp.cds
                )
                the_one_existing_temp.delete()
                return
            ...
            args[
                "info_to_contact"
            ] = "Erreur. Vous avez envoye le numero de telephone du superviseur de differentes manieres. Veuillez reenvoyer le message commencant par rg. Merci"
...
def record_sds(args):
    ...
    the_created_report, created = Report.objects.get_or_create(
        cds=args["cds"],
        concerned_date=args["sent_date"],
        category="STOCK_DEBUT_SEMAINE",
    )
    ...
    while priority <= args["number_of_concerned_products"]:
        ...
        prod_camp = CampaignProduct.objects.filter(
            campaign=args["opened_campaign"], order_in_sms=priority
        )

        the_concerned_prod_campaign = prod_camp[0]

        report_prod, created = ReportProductReception.objects.get_or_create(
            campaign_product=the_concerned_prod_campaign,
            reception_date=args["sent_date"],
            report__cds=args["cds"],
            report__category="STOCK_DEBUT_SEMAINE",
        )
        report_prod.received_quantity, report_prod.report = (
            value.replace(",", "."),
            the_created_report,
        )
        report_prod.save()
        ...
        if priority == 1:
            message_to_send = (
                message_to_send
                + ""
                + the_concerned_prod_campaign.product.name
                + " : "
                + value
            )
        else:
...
def record_sr(args):
    ...
    # Let's record the a beneficiary report
    the_created_report, created = Report.objects.get_or_create(
        cds=args["cds"], concerned_date=args["sent_date"], category="STOCK_RECU"
    )
    the_created_report.text = args["text"]
    the_created_report.save()
...
def record_sf(args):
    ...
    the_created_report, created = Report.objects.get_or_create(
        cds=args["cds"], concerned_date=args["sent_date"], category="STOCK_FINAL"
    )
    the_created_report.text = args["text"]
    the_created_report.save()
    ...
    while priority <= args["number_of_concerned_products"]:
        ...
        report_prod, created = ReportProductRemainStock.objects.get_or_create(
            campaign_product=the_concerned_prod_campaign,
            concerned_date=args["sent_date"],
            report__cds=args["cds"],
        )
        report_prod.remain_quantity, report_prod.report = (
            value.replace(",", "."),
            the_created_report,
        )
        report_prod.save()
...
def identify_number_of_concerned_beneficiaries(args):
    ...
    campaign_beneficiaries = CampaignBeneficiary.objects.filter(
        campaign=args["opened_campaign"]
    )
    ...
    if len(campaign_beneficiaries) < 1:
        ...
        args[
            "info_to_contact"
        ] = "Erreur admin. Pas de beneficiaires lies a la campagne ouverte."
        return
    ...
    for campaign_beneficiary in campaign_beneficiaries:
        if args["valide"]:
            camp_ben_products = CampaignBeneficiaryProduct.objects.filter(
                campaign_beneficiary=campaign_beneficiary
            )
            if len(camp_ben_products) < 1:
                ...
                args[
                    "info_to_contact"
                ] = "Erreur admin. Il y a des beneficiaires dont les produits a recevoir ne sont pas defini."
            else:
                number_of_expected_beneficiarie_values = (
                    number_of_expected_beneficiarie_values + len(camp_ben_products)
                )
...
def record_beneficiaries(args):
    ...
    # Let's record the a beneficiary report
    the_created_report, created = Report.objects.get_or_create(
        cds=args["cds"], concerned_date=args["sent_date"], category="BENEFICIAIRE"
    )
    the_created_report.text = args["text"]
    the_created_report.save()
    ...
    while priority <= args["number_of_beneficiries_per_product"]:
        # We record each beneficiary number
        value = args["text"].split()[priority + 1]
        camp_ben_prod = CampaignBeneficiaryProduct.objects.filter(
            campaign_beneficiary__campaign=args["opened_campaign"],
            order_in_sms=priority,
        )
        ...
        if priority == 1:
            message_to_send = (
                message_to_send
                + ""
                + the_concerned_camp_ben_prod.campaign_beneficiary.beneficiary.designation
                + " : "
                + value
            )
        else:
        ...
        report_ben, created = ReportBeneficiary.objects.get_or_create(
            beneficiaries_per_product=the_concerned_camp_ben_prod,
            campaign_beneficiary=the_concerned_ben_campaign,
            reception_date=args["sent_date"],
            report__cds=args["cds"],
        )
        report_ben.received_number, report_ben.report = value, the_created_report
...
def check_stock_out_values_validity(args):
    ...
    if len(campaign_product) < 1:
        args["valide"] = False
        args["info_to_contact"] = (
            "Erreur. Pas de produits de priorite "
            + str(priority)
            + ". Veuillez reenvoyer le message corrige."
        )
    else:
        one_campaign_product = campaign_product[0]
        ...
        if re.search(expression, value) is None:
            ...
            args[
                "info_to_contact"
            ] = "Erreur. La quantite restante envoye n est pas valide."
...
def alert_for_stock_out(args):
    ...
    if args["the_sender"].supervisor_phone_number:
        ...
        data = {
            "urns": [the_sup_phone_number],
            "text": args["message_to_send_for_alert"],
        }
    ...
    concerned_district_supersors = DistrictSupervisor.objects.filter(
        district=the_concerned_district
    )
...
def record_stock_out(args):
    ...
    the_created_report = Report.objects.create(
        ...
        concerned_date=datetime.datetime.now().date(),
        text=args["text"],
        category="RUPTURE_STOCK",
    )
    ...
    if len(prod_camp) < 1:
        ...
        args[
            "info_to_contact"
        ] = "Erreur d enregistrement du rapport de rupture du stock."
        return
    else:
        prod_camp = prod_camp[0]
        stock_out_report_object = ReportStockOut.objects.create(
            campaign_product=prod_camp,
            remaining_stock=value.replace(",", "."),
        ...
        args[
            "measuring_unit"
        ] = stock_out_report_object.campaign_product.product.unite_de_mesure
        args["message_to_send_for_alert"] = (
            "Une rupture de stock de "
            + args["product_name"]
            + " est signalee a "
            + args["cds_name"]
            + ", "
        ...
        args["info_to_contact"] = (
            "Le rapport de rupture de stock de '"
            + args["product_name"]
            + "', lieu '"
            + args["cds_name"]
            + "' est bien enregistre."
...
def check_beneficiaries_value(args):
    ...
    if re.search(expression, value) is None:
        ...
        args[
            "info_to_contact"
        ] = "Erreur. La valeur envoyee pour la population cible n est pas valide."
    else:
        args["valide"] = True
        args[
            "info_to_contact"
        ] = "Le nombre envoye pour la population cible est valide."
...
def record_population_cible(args):
    ...
    the_eventuals_existing_camp_cds = CampaignCDS.objects.filter(
        campaign=the_campaign, cds=the_cds
    )
    ...
    if len(the_eventuals_existing_camp_cds) > 0:
        ...
        the_new_camp_cds = CampaignCDS.objects.create(
            campaign=the_campaign, cds=the_cds, population_cible=value
        )
