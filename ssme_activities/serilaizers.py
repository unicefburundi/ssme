...
if not days:
    days = [
        {
            "end_date": datetime.date(2017, 12, 6),
...
campaigndays = (
    datetime.datetime.strptime(days[0]["end_date"].isoformat(), "%Y-%m-%d").date()
    - datetime.datetime.strptime(days[0]["start_date"].isoformat(), "%Y-%m-%d").date()
).days
...
def get_report_by_code(request, code, model):
    ...
    if len(code) <= 2:
        return queryset.filter(
            report__cds__district__province__code=int(code)
        ).order_by("report__concerned_date")
    if len(code) > 2 and len(code) <= 4:
        return queryset.filter(report__cds__district__code=int(code)).order_by(
            "report__concerned_date"
        )
    if len(code) > 4:
        return queryset.filter(report__cds__code=code).order_by(
            "report__concerned_date"
        )
...
class ProvinceSerializer(serializers.ModelSerializer):
    ...
    def get_stock_debut_semaine(self, obj):
        rappors = Report.objects.filter(
            category="STOCK_DEBUT_SEMAINE",
            cds__district__province=obj,
            concerned_date__range=[days[0]["start_date"], days[0]["end_date"]],
        ).distinct()
...
class CampaignSerializer(serializers.ModelSerializer):
    ...
    def get_days(self, obj):
        lesdates = [
            ...
            for i in range(obj.start_date.toordinal(), obj.end_date.toordinal() + 1)
        ]
        return dict(list(enumerate(lesdates)))

    def get_benefs(self, obj):
        dates_benef = (
            ReportBeneficiary.objects.values("reception_date")
            .distinct()
            .order_by("reception_date")
        )
        queryset_benef = get_report_by_code(
            self.context.get("request"), self.context.get("mycode"), ReportBeneficiary
        )
        body_benef = []
        headers_benef = (
            ...
            for t in headers_benef:
                ress = (
                    ...
                    .filter(
                        ...
                        beneficiaries_per_product=t["id"],
                    )
